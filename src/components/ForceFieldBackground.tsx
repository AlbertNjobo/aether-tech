...
export function ForceFieldBackground({
  ...
  useEffect(() => {
    ...
    const sketch = (p: p5) => {
      ...
      p.draw = () => {
        if (!img) return;
        p.clear(); // Transparent background so we can layer it
  ...
  return (
    ...
    >
      {isLoading && (
        <div className="absolute inset-0 flex items-center justify-center text-white/50 text-xs tracking-widest uppercase z-50">
          <div className="w-1 h-1 bg-red-500 rounded-full animate-ping mr-2" />
          Initializing Force Field...
        </div>
      )}
      {error && (
        <div className="absolute inset-0 flex items-center justify-center text-red-500/50 text-xs tracking-widest uppercase z-50">
          {error}
        </div>
      )}
    </div>
